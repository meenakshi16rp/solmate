    ...
    try:
        db.execute(
            '''INSERT INTO responses 
               (location, roof_area, electricity_consumption, electricity_rate, storage_option, budget, submitted_at)
               VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (location, roof_area, electricity_consumption, electricity_rate, storage_option, budget, datetime.datetime.now().isoformat())
        )
        db.commit()
...
def final_report():
    ...
    monthly_unit_consumption =float(electricity_consumption) /float(electricity_rate) 
    daily_unit_consumption = monthly_unit_consumption / 30.0# may be provided
    storage_option = user_data['storage_option']           # boolean: True for battery storage, False for grid offloading
    ...
    battery_cost_per_kw = 20000 if  storage_option== 'battery' else 0
    ...
    if storage_option=='battery':
        ...
        consumption_savings = float(annual_consumption) * float(electricity_rate)
